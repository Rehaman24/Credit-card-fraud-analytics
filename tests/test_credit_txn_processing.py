...
def spark():
    """Initialize a Spark Session for Testing with proper teardown."""
    ...
    yield spark  # Provide the spark session to the test
    
    # Teardown: Stop the session after all tests in the module run
    spark.stop()
...
def test_transactions_processing(spark):
    ...
    result = enriched_df.select(
        "transaction_id", "transaction_category", "high_risk", 
        "fraud_risk_level", "updated_reward_points"
    ).collect()
    
    # Assertions for T001
    assert result[0]["transaction_category"] == "Medium"
    assert result[0]["high_risk"] == False
    assert result[0]["fraud_risk_level"] == "Low"
    assert result[0]["updated_reward_points"] == 4512

    # Assertions for T002
    assert result[1]["transaction_category"] == "High"
    assert result[1]["high_risk"] == True
    assert result[1]["fraud_risk_level"] == "Critical"
    assert result[1]["updated_reward_points"] == 2150

    # Assertions for T003
    assert result[2]["transaction_category"] == "Low"
    assert result[2]["high_risk"] == False
    assert result[2]["fraud_risk_level"] == "Low"
    assert result[2]["updated_reward_points"] == 8008
