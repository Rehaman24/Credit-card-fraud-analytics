...
def spark():
    ...
    print("\n--- SPARK FIXTURE: Initializing Spark Session ---")
    spark = SparkSession.builder \
        .appName("PySparkUnitTest") \
        .master("local[*]") \
        .getOrCreate()
    
    print("--- SPARK FIXTURE: Spark Session Created ---")
    
    yield spark  # Provide the spark session to the test
    
    print("\n--- SPARK FIXTURE: Stopping Spark Session ---")
    spark.stop()
    print("--- SPARK FIXTURE: Spark Session Stopped ---")

def test_transactions_processing(spark):
    """Test transaction processing logic"""
    print("\n--- TEST: test_transactions_processing STARTED ---")
    ...
    print("--- TEST: COLLECT action started (this may take a moment)... ---")
    result = enriched_df.select("transaction_id", "transaction_category", "high_risk", "fraud_risk_level", "updated_reward_points").collect()
    print("--- TEST: COLLECT action finished ---")

    print("--- TEST: Assertions started ---")
